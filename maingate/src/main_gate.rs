...
#[derive(Clone, Debug)]
pub struct MainGate<F: Field> {
    config: MainGateConfig,
    _marker: PhantomData<F>,
}

impl<F: Field> Chip<F> for MainGate<F> {
...
#[derive(Clone, Debug)]
pub enum CombinationOption<F: Field> {
...
impl<F: Field> From<CombinationOptionCommon<F>> for CombinationOption<F> {
...
impl<F: Field> MainGate<F> {
    ...
    pub fn configure(
        ...
        meta.create_gate("main_gate", |meta| {
            ...
            let sd = meta.query_fixed(sd);
            let se = meta.query_fixed(se);

            let se_next = meta.query_fixed(se_next);

            let s_mul_ab = meta.query_fixed(s_mul_ab);
            let s_mul_cd = meta.query_fixed(s_mul_cd);

            let s_constant = meta.query_fixed(s_constant);
...
mod tests {
    ...
    impl TestCircuitConfig {
        fn main_gate<F: Field>(&self) -> MainGate<F> {
            MainGate::<F> {
    ...
    #[derive(Default)]
    struct TestCircuitPublicInputs<F: Field> {
        _marker: PhantomData<F>,
        public_input: F,
    }
    ...
    #[derive(Default)]
    struct TestCircuitCombination<F: Field> {
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitBitness<F: Field> {
        neg_path: bool,
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitEquality<F: Field> {
        neg_path: bool,
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitArith<F: Field> {
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitConditionals<F: Field> {
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitDecomposition<F: Field> {
        _marker: PhantomData<F>,
    ...
    #[derive(Default)]
    struct TestCircuitComposition<F: Field> {
        _marker: PhantomData<F>,
    }
    ...
    #[derive(Default)]
    struct TestCircuitSign<F: Field> {
        _marker: PhantomData<F>,
    }
