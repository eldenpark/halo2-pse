...
mod tests {
    ...
    fn verify_constants33() {
        fn verify_constants_helper(
            expected_round_constants: [[SecFq; 3]; 64],
            expected_mds: [[SecFq; 3]; 3],
            expected_mds_inv: [[SecFq; 3]; 3],
        ) {
            let (round_constants, mds, mds_inv) = P128Pow5T3Gen::<SecFq, 0>::constants();

            // let mut file = File::create("secp_fp_inv").unwrap();
