...
mod tests {
    ...
    use crate::poseidon::{
        primitives::{
            self as poseidon, ConstantLength, P128Pow5T3 as OrchardNullifier, P128Pow5T3Sec, Spec,
        },
        Hash,
    };
    use std::convert::TryInto;
    ...
    struct HashCircuitSec<
        S: Spec<SecFp, WIDTH, RATE>,
        const WIDTH: usize,
        const RATE: usize,
        const L: usize,
    > {
        message: Value<[SecFp; L]>,
        // For the purpose of this test, witness the result.
        // TODO: Move this into an instance column.
        output: Value<SecFp>,
        _spec: PhantomData<S>,
    ...
    impl<S: Spec<SecFp, WIDTH, RATE>, const WIDTH: usize, const RATE: usize, const L: usize>
        Circuit<SecFp> for HashCircuitSec<S, WIDTH, RATE, L>
    {
        type Config = Pow5Config<SecFp, WIDTH, RATE>;
        type FloorPlanner = SimpleFloorPlanner;
        ...
        fn configure(meta: &mut ConstraintSystem<SecFp>) -> Pow5Config<SecFp, WIDTH, RATE> {
            let state = (0..WIDTH).map(|_| meta.advice_column()).collect::<Vec<_>>();
            let partial_sbox = meta.advice_column();
        ...
        fn synthesize(
            &self,
            config: Pow5Config<SecFp, WIDTH, RATE>,
            mut layouter: impl Layouter<SecFp>,
        ) -> Result<(), Error> {
            let chip = Pow5Chip::construct(config.clone());
            ...
            let output = hasher.hash(layouter.namespace(|| "hash"), message)?;
            println!("in-circuit, output: {:?}", output);
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    println!("in-circuit, expected_var: {:?}", expected_var);
    ...
    fn poseidon_hash2() {
        ...
        let message = [SecFp::random(rng), SecFp::random(rng)];
        let output =
            poseidon::Hash::<_, P128Pow5T3Sec, ConstantLength<2>, 3, 2>::init().hash(message);

        println!("11 output: {:?}", output);

        let k = 6;
        let circuit = HashCircuitSec::<P128Pow5T3Sec, 3, 2, 2> {
