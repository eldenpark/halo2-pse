use crate::halo2::arithmetic::{CurveAffine, Field};
...
#[derive(Clone, Debug)]
pub struct Point<W: Field, N: Field, const NUMBER_OF_LIMBS: usize, const BIT_LEN_LIMB: usize> {
...
/// algorithm
#[derive(Default)]
pub(crate) struct Selector<F: Field>(Vec<AssignedCondition<F>>);

impl<F: Field> fmt::Debug for Selector<F> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
pub(crate) struct Windowed<F: Field>(Vec<Selector<F>>);

impl<F: Field> fmt::Debug for Windowed<F> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
pub(crate) struct Table<W: Field, N: Field, const NUMBER_OF_LIMBS: usize, const BIT_LEN_LIMB: usize>(
